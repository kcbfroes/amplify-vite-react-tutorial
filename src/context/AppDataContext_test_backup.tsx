...
type ATodo = { id: string; content: string; isDone: boolean; ownerId: string; assignedToId: string }
...
const mockClient = generateClient<Schema>() as {
    models: {
      Todo: {
        observeQuery: () => {
            subscribe: (callback: (data: subscribeData) => void) => { unsubscribe: () => void; };
        };
      };
    };
  };
...
const mockObserveQuery: () => 
    {
        ...
        subscribe: (callback: Function) => {
            callback({
                items: [{ id: '1', content: 'Fake Task 1', isDone: false, ownerId: 'ABC', assignedToId: 'def' }],
                isSynced: true,
            });
...
vi.spyOn(mockClient.models.Todo, 'observeQuery').mockImplementation(mockObserveQuery);
...
describe('AppDataContext', () => {
    it('provides initial context values correctly', async () => {
        await act(async () => { render(<AppDataProvider><TestComponent /></AppDataProvider>) });
